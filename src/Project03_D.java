    ...
    private static void getIsbnImage(ExcelVO vo) {
        try {
            String openapi = "https://openapi.naver.com/v1/search/book_adv.xml?d_titl="
                    + URLEncoder.encode(vo.getTitle(),"UTF-8")+"&d_auth="+URLEncoder.encode(vo.getAuthor(),"UTF-8")
                    + "&d_publ="+ URLEncoder.encode(vo.getTitle(),"UTF-8");
            URL url = new URL(openapi);
            HttpURLConnection con =(HttpURLConnection) url.openConnection();
            con.setRequestMethod("GET");
            con.setRequestProperty("X-Naver-Client-Id","LTBeAi2rNINHdgWTrSN8");
            con.setRequestProperty("X-Naver-Client-Secret","073mjmdzSc");
